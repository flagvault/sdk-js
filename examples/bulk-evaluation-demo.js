...
async function demonstrateBulkEvaluation() {
  ...
  try {
    ...
    const rolloutFlag = Array.from(allFlags.values()).find(
      (f) => f.rolloutPercentage != null,
    );
    ...
    if (rolloutFlag) {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        console.log(
          `- Attempt ${i + 1}: ${enabled ? "✓ Enabled" : "✗ Disabled"}`,
        );
      }
    } else {
    ...
    console.log(
      `✓ Evaluated ${allFlags.size} flags in ${elapsed}ms (using cache)`,
    );
  } catch (error) {
